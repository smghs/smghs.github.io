...
function App({match}) {
  ...
  return(
     ...
     <Header title='학년게시판' link='/boardlist'/>
     <BoardContainer>
     {view.map((element) =>
         ...
         <Comments><FaRegComment style={{'font-size':'0.9rem','margin-right':'0.1rem'}}/>{element.replys.length}</Comments>
          <Hearts><BiLike style={{'margin-right':'0.1rem'}}/>{element.likes.length}</Hearts>
         </BoardBlock>
        )}
        </BoardContainer>
        <WriteBlock onClick={()=>{window.location.href='/write/grade'}}>
